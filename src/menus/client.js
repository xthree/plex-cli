 ...
 * @param {object} plexClient  Client metadata object from the API
 */
export async function runClientMenu(client, plexClient) {
  ...
  while (true) {
    ...
    if (action === 'player') {
      ...
      try {
        ...
        const offset = parseInt(tl.viewOffset ?? 0, 10) + 30000;
      ...
      try {
        const tl = await client.getTimeline(address, port);
        const offset = Math.max(0, parseInt(tl.viewOffset ?? 0, 10) - 30000);
        await client.playerCommand(address, port, 'seekTo', { offset });
        printSuccess(`⏮ Jumped to ${fmtDuration(offset)}.`);
      } catch (err) { printError(err.message); }

    } else if (action === 'seek') {
      const { timeStr } = await inquirer.prompt([
        {
          type: 'input',
          name: 'timeStr',
      ...
      try {
        ...
        const offset = parseInt(tl.viewOffset ?? 0, 10);
        const duration = parseInt(tl.duration ?? 0, 10);
        console.log(`\n  ${full}`);
