...
export async function runLibrariesMenu(client) {
...
async function browseLibrary(client, lib) {
  ...
  while (true) {
    ...
    const navChoices = items.map((it) => ({ name: it.title ?? '?', value: it }));
    ...
    await runMediaMenu(client, chosen);
  }
}
